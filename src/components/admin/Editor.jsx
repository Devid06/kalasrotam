...
import { fetchLeads, deleteLead, downloadCsv } from '../../lib/leads.js'
...
/* ============================================================================
   ADMIN PANEL  —  /?admin=1
   ----------------------------------------------------------------------------
   Edit any text, price or image and watch the page behind update as you type.
   Press Publish and it writes straight to the live site — every visitor sees it
   within about a second, with no file to download or upload.

   WHY EDITS ARE NOT SAVED AS YOU TYPE
   Publishing is instant, but it is still deliberate. Autosaving to a live site
   would push half-typed headlines in front of whoever happens to be reading the
   page at that moment. So changes preview locally, and Publish is the moment
   they go out. It is one button and it takes a second.

   ON ACCESS
   ?admin=1 only decides whether the panel is visible; the login decides whether
   anything can be saved. The real enforcement lives in the database's Row Level
   Security policies, so even a forced-open panel cannot write without a valid
   session.
   ========================================================================== */
...
export default function Editor() {
  ...
  const { isSignedIn, checking, signIn, signOut, user } = useAuth()

  // Reaching this component at all means the editor was asked for, so this is
  // the point where the live socket is worth opening.
  useEffect(() => {
  ...
  const payload = useMemo(
    () => buildPublishPayload(publishedLayer, remote, draft),
    [publishedLayer, remote, draft]
  )
  const size = useMemo(() => payloadSize(payload), [payload])

  const flash = (kind, text, ms = 5000) => {
    setStatus({ kind, text })
  ...
  const doPublish = async () => {
    ...
    try {
      await publish()
      flash('ok', 'Published. The live site is updated for everyone.')
    } catch (err) {
      flash('err', 'Could not publish: ' + (err.message || 'unknown error'), 9000)
    } finally {
      setPublishing(false)
    }
  }
  ...
  const importBackup = async (e) => {
    ...
    try {
      ...
      // Merged over the current draft rather than replacing it, so restoring a
      // backup does not silently discard edits made since.
      setDraft(merge(draft, parsed))
      flash('ok', 'Backup loaded as unpublished changes. Press Publish to make it live.')
    } catch {
      flash('err', 'That file could not be read as a backup.')
    }
    e.target.value = ''
  }
  ...
  // Signing in is required before anything can be published. The panel still
  // opens when Supabase is unconfigured, so the site can be worked on before
  // the backend exists.
  if (isConfigured && !isSignedIn) {
    if (checking) {
      return createPortal(
        <aside className="admin admin--login">
          <div className="admin__body">
            <p className="af__hint">Checking your session...</p>
          </div>
        </aside>,
        document.body
      )
    }
    return createPortal(<Login onSignIn={signIn} onClose={() => setOpen(false)} />, document.body)
  }

  const canPublish = isConfigured && isSignedIn && hasDraft
  ...
  return createPortal(
    <aside className="admin" aria-label="Site editor">
      <header className="admin__bar">
        <div>
          ...
          <span className="admin__sub">
            {hasDraft ? 'Unpublished changes' : 'Everything published'} &middot; {formatBytes(size)}
            {user?.email ? ' · ' + user.email : ''}
          </span>
        </div>
        <div className="admin__bar-tools">
          {isSignedIn && (
            <button type="button" onClick={signOut} className="admin__x admin__signout" title="Sign out">
              Sign out
            </button>
          )}
      ...
      <div className="admin__body">
        {!isConfigured && (
          <p className="admin__warn">
            Not connected to the database. You can preview changes on this device, but nothing can be published until
            the Supabase keys are set.
          </p>
        )}
        {isConfigured && !liveContentAvailable && (
          <p className="admin__warn">
            The database could not be reached when this page loaded, so you are editing the last published copy.
            Reload before publishing, or you may overwrite newer changes.
          </p>
        )}
      ...
      <footer className="admin__foot">
        {status && (
          <p className={status.kind === 'ok' ? 'admin__flash' : 'admin__warn'} role="status">
            {status.text}
          </p>
        )}
        <div className="admin__actions">
          <button type="button" className="abtn abtn--primary" onClick={doPublish} disabled={!canPublish || publishing}>
            {publishing ? 'Publishing...' : hasDraft ? 'Publish changes' : 'Nothing to publish'}
          </button>
          <button
            type="button"
            className="abtn abtn--ghost"
            onClick={() => downloadContentJson(payload)}
...
function LeadsTab() {
  ...
  const remove = async (lead) => {
    if (!confirm('Delete the enquiry from ' + (lead.name || 'this person') + '?')) return
    try {
  ...
  return (
    <>
      {source === 'database' ? (
        <p className="admin__note">
          Every enquiry and signup from the live site lands here. Export the CSV to keep them, or to import them into
          your phone contacts.
        </p>
      ) : (
        <p className="admin__warn">
          Showing enquiries saved in <strong>this browser only</strong> — the database is not connected, so real
          visitors&rsquo; details are not reaching you. {error ? 'Error: ' + error : ''}
        </p>
      )}

      <Group title={loading ? 'Loading…' : 'Enquiries (' + leads.length + ')'}>
        {!loading && leads.length === 0 && <p className="af__hint">Nothing yet.</p>}

        {leads.map((l) => (
          <div className="alead" key={l.id}>
            <div className="alead__head">
              <b>{l.name || 'No name'}</b>
              <button type="button" className="alead__del" onClick={() => remove(l)} aria-label="Delete enquiry">
                &times;
              </button>
            </div>
            <span>
              {l.phone ? (
                <a href={'https://wa.me/' + String(l.phone).replace(/\D/g, '').replace(/^(?!91)/, '91')} target="_blank" rel="noopener noreferrer">
                  {l.phone}
                </a>
              ) : null}
              {l.email ? (
            ...
            <span>
              {l.type} · {new Date(l.at || l.created_at).toLocaleString('en-IN')}
            </span>
            {l.medium && (
              <span>
                {l.medium}
            ...
            {l.message && <span>&ldquo;{l.message}&rdquo;</span>}
          </div>
        ))}
        ...
        <div className="admin__actions">
          ...
          <button type="button" className="abtn abtn--sm abtn--ghost" onClick={load} disabled={loading}>
            Refresh
          </button>
        </div>
      </Group>
